...
// --- Reverted Form Helper Components ---
...
export function AdminDashboard() {
  ...
  // --- GENERAL REGISTRY PDF (INCLUDES SPORT + SERIAL NUMBERS) ---
  const downloadGeneralRegistry = () => {
    ...
    const tableData = athletes.map((a: any, index: number) => [
      ...
      a.athleteId,
      `${a.firstName} ${a.middleName || ''} ${a.lastName}`.replace(/\s+/g, ' ').toUpperCase(),
      a.sport.toUpperCase(),
      a.gender.toUpperCase(),
      a.dob,
      a.parentPhone,
      a.homeAddress.toUpperCase()
    ]);

    autoTable(doc, {
      startY: 30,
      head: [['S/N', 'REG ID', 'FULL NAME', 'SPORT', 'GENDER', 'DOB', 'PHONE', 'ADDRESS']],
      body: tableData,
      headStyles: { fillColor: [30, 58, 138], fontSize: 8 },
      bodyStyles: { fontSize: 7 }
    });

    doc.save(`IYSDC_Registry_Sport_Master.pdf`);
  };

  // --- PORTRAIT ID PDF (NO SPORT - ONLY PERSONAL INFO) ---
  const downloadPortraitID = (a: any) => {
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: [53.9, 85.6] });
    
    // Header
    doc.setFillColor(30, 58, 138); 
    doc.rect(0, 0, 53.9, 22, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8); doc.setFont('helvetica', 'bold');
    doc.text("IYSDC OFFICIAL", 26.95, 9, { align: 'center' });
    doc.setFontSize(5); doc.text("ATHLETE CREDENTIAL 2026", 26.95, 13, { align: 'center' });

    // Photo
    if (a.photo) {
        doc.addImage(a.photo, 'JPEG', 14.95, 16, 24, 24);
    }

    let y = 46;
    const drawField = (label: string, val: string, halfWidth = false, xOffset = 6) => {
      doc.setFontSize(4); doc.setTextColor(100, 116, 139); doc.text(label.toUpperCase(), xOffset, y);
      doc.setFontSize(6.5); doc.setTextColor(15, 23, 42); 
      const wrapped = doc.splitTextToSize((val || "N/A").toUpperCase(), halfWidth ? 20 : 42);
      doc.text(wrapped, xOffset, y + 3);
      if (!halfWidth) y += (wrapped.length > 1 ? 8.5 : 7.5);
    };

    const fullName = `${a.firstName} ${a.middleName || ''} ${a.lastName}`.replace(/\s+/g, ' ').trim();
    
    drawField("Full Name", fullName);
    drawField("Registration ID", a.athleteId);
    
    // Row for DOB and Gender
    drawField("Date of Birth", a.dob, true, 6);
    drawField("Gender", a.gender, true, 30);
    y += 7.5;

    drawField("Guardian Contact", a.parentPhone);
    drawField("Residential Address", a.homeAddress);
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <aside className="lg:col-span-4">
          <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-xl sticky top-28">
            <h2 className="text-xs font-black uppercase tracking-widest mb-6 flex items-center gap-2">
              <UserPlus size={18} className="text-amber-500" /> ID Card Registry
            </h2>
            <form onSubmit={onRegister} className="space-y-4">
              <div className="flex flex-col items-center mb-4">
                <label className="w-24 h-24 rounded-2xl bg-slate-50 border-2 border-dashed flex items-center justify-center cursor-pointer overflow-hidden">
        ...
        <main className="lg:col-span-8 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button onClick={downloadGeneralRegistry} className="bg-emerald-600 text-white px-8 py-4 rounded-3xl text-[10px] font-black uppercase flex items-center gap-2 shadow-lg">
              <Download size={16} /> Registry Download (S/N)
            </button>
          </div>

          {athletes?.filter(a => `${a.firstName} ${a.lastName}`.toLowerCase().includes(searchTerm.toLowerCase())).map((a: any, idx: number) => (
            <div key={a.id} className="bg-white rounded-[2rem] border border-slate-200 hover:border-amber-400 transition-all shadow-sm">
              {editingId === a.id ? (
                <div className="p-8 grid grid-cols-2 gap-4">
                  ...
                  <button onClick={() => { updateDoc(doc(db, 'athletes', a.id), editData); setEditingId(null); }} className="col-span-1 py-3 bg-slate-900 text-white rounded-lg text-[10px] font-black">SAVE</button>
                  <button onClick={() => setEditingId(null)} className="col-span-1 py-3 bg-slate-200 text-slate-600 rounded-lg text-[10px] font-black">CANCEL</button>
                </div>
              ) : (
                <div className="p-6 flex items-center gap-6">
                  <div className="relative">
                    <img src={a.photo} className="w-20 h-20 rounded-2xl object-cover border" />
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-bold text-slate-400">{a.athleteId}</span>
                      <span className="text-[8px] px-2 py-0.5 bg-slate-100 rounded-full font-black text-slate-500 uppercase">{a.sport}</span>
                    </div>
                    <h3 className="font-black text-slate-900 uppercase text-lg leading-tight">{a.firstName} {a.middleName} {a.lastName}</h3>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => downloadPortraitID(a)} className="bg-slate-900 text-white p-3 rounded-2xl shadow-sm"><FileText size={20} /></button>
                    <button onClick={() => { setEditingId(a.id); setEditData(a); }} className="bg-slate-100 text-slate-600 p-3 rounded-2xl"><Edit2 size={20} /></button>
                    <button onClick={() => window.confirm("Delete?") && deleteDoc(doc(db, 'athletes', a.id))} className="bg-slate-50 text-slate-200 p-3 rounded-2xl hover:text-red-500"><Trash2 size={20} /></button>
                  </div>
                </div>
              )}
            </div>
